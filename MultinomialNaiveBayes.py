...
def loadCSVwithPandas(n, data_dir=data_dir):
	...
	X=pd.read_csv(data_dir+file_name, header=None, usecols=list(range(col_num-1)), dtype=np.int32).as_matrix()
...
def SparseMatrix(n, data_dir=data_dir):
	...
	with open(data_dir+file_name,'r',encoding="utf-8") as f:
		...
		X_sparse_matrix=lil_matrix((row_num,col_num-1),dtype=np.int32)
		y_sparse_matrix=[]
...
def MultinomialNaiveBayes(k=k, n_list=n_list):
	# Wipe out previous saved file
	f=open(result_dir+"NB_classification_result.txt", 'w')
	f.close()
	...
	for n in n_list:
		...
		for turn in range(k):
			...
			f=open(result_dir+"NB_classification_result.txt", 'a')
